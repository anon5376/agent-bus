...
const OPEN_EXECUTION_STATES = ["assigned", "in_progress", "submitted", "changes_requested"];
...
function contextReferences(value) {
    if (!Array.isArray(value))
        return [];
    return value.slice(0, 100).map((item) => {
        const row = item && typeof item === "object" ? item : { value: String(item) };
        const type = String(row.type ?? "path");
        if (!["path", "artifact", "summary", "commit", "url"].includes(type))
            throw new Error(`invalid context reference type: ${type}`);
        return {
            type: type,
...
function validationRequirements(value) {
    if (!Array.isArray(value))
        return [];
    return value.slice(0, 50).map((item, index) => {
        const row = item && typeof item === "object" ? item : { description: String(item) };
        return {
...
function resultPayload(body) {
    ...
    const validation = Array.isArray(body.validation)
        ? body.validation.slice(0, 100).map((item) => {
            const row = item && typeof item === "object" ? item : { summary: String(item) };
            return {
...
export class BrokerService {
    ...
    ensureConfiguredRoster() {
        for (const resolvedAgent of enabledAgents(this.config)) {
            if (this.agents.has(resolvedAgent.id))
                continue;
    ...
    agentFromDefinition(definition, registeredAt) {
        return {
            id: definition.id,
    ...
    requireOperator(body) {
        ...
        if (identity.authority !== "operator")
            throw new PermissionError("only the operator may perform this action");
        return identity;
    }
    touch(agentId, status) {
    ...
    effectiveStatus(agent) {
        if (agent.id === OPERATOR_ID)
            return agent.status;
        if (!agent.registeredAt || !agent.lastSeen)
            return "offline";
    ...
    taskProjectRoot(task) {
        if (task.runId)
            return this.runs.get(task.runId)?.projectRoot ?? null;
    ...
    addEvent(task, actor, kind, state, note, metadata) {
        task.history.push({ ts: Date.now(), actor, kind, state, note, metadata });
        task.updatedAt = Date.now();
    }
    tryDispatch(task, reason = "ready") {
        if (TERMINAL_STATES.includes(task.state) || task.state === "submitted")
            return false;
        ...
        const assigneeBusy = [...this.tasks.values()].some((candidate) => candidate.id !== task.id && candidate.assignee === task.assignee && OPEN_EXECUTION_STATES.includes(candidate.state));
        if (assigneeBusy) {
            task.state = "blocked";
        ...
        if (!task.readOnly && this.config.constraints.isolation === "path-locks") {
            ...
            if (!projectRoot)
                throw new Error(`cannot acquire path leases without a project root for ${task.id}`);
            ...
            if (!lease.acquired) {
                task.state = "blocked";
                this.addEvent(task, "broker", "blocked", "blocked", `path scope conflict with ${lease.conflicts.map((item) => `${item.taskId}:${item.path}`).join(", ")}`, { conflicts: lease.conflicts });
                this.store.saveTask(task);
                return false;
            }
        }
    ...
    refreshBlocked(runId) {
        ...
        for (const task of blocked) {
            if (this.activeExecutionCount() >= this.config.constraints.maxConcurrentTasks)
                break;
    ...
    completeRunIfReady(task) {
        if (!task.runId)
            return;
        const run = this.runs.get(task.runId);
        if (!run || run.rootTaskId !== task.id || task.state !== "accepted")
            return;
    ...
    createTask(identity, body) {
        ...
        for (const dependency of dependencyIds) {
            ...
            if (runId && task.runId !== runId)
                throw new Error(`dependency ${dependency} belongs to a different run`);
        }
        const now = Date.now();
    ...
    async handle(path, body) {
        switch (path) {
            case "/agent/provision": {
                ...
                if (!id || id === OPERATOR_ID)
                    throw new Error("a configured non-operator agent id is required");
            ...
            case "/register": {
                ...
                const id = String(body.id ?? "");
                if (id !== identity.id)
                    throw new AuthError(`token belongs to ${identity.id}, not ${id || "(missing id)"}`);
                if (id === OPERATOR_ID)
                    throw new AuthError("operator identity cannot register as an agent");
                const definition = resolveAgent(this.config, id);
            ...
            case "/status": {
                ...
                if (!agent)
                    throw new Error(`unknown registered agent: ${identity.id}`);
            ...
            case "/agent/execution-config": {
                ...
                if (identity.authority !== "operator" && id !== identity.id)
            ...
            case "/run/get": {
                ...
                if (!run)
                    throw new Error(`unknown run: ${body.runId}`);
            ...
            case "/task/start": {
                ...
                if (identity.id !== task.assignee)
                    throw new PermissionError(`only ${task.assignee} may start ${task.id}`);
            ...
            case "/task/submit": {
                ...
                if (!task)
                    throw new Error(`unknown task: ${body.taskId}`);
                if (identity.id !== task.assignee && identity.id !== OPERATOR_ID)
                    throw new PermissionError(`only ${task.assignee} may submit ${task.id}`);
            ...
            case "/task/review": {
                ...
                if (task.state !== "submitted")
                    throw new ConflictError(`task ${task.id} is ${task.state}, not submitted`);
            ...
            case "/task/failure": {
                ...
                if (!task)
                    throw new Error(`unknown task: ${body.taskId}`);
                if (identity.id !== task.assignee)
                    throw new PermissionError(`only ${task.assignee} may report failure for ${task.id}`);
                task.attempts += 1;
            ...
            case "/task/cancel": {
                ...
                if (!task)
                    throw new Error(`unknown task: ${body.taskId}`);
                if (identity.id !== OPERATOR_ID && identity.id !== task.assigner)
                    throw new PermissionError(`only ${task.assigner} or operator may cancel ${task.id}`);
                if (TERMINAL_STATES.includes(task.state))
                    return { task: this.taskView(task) };
            ...
            case "/kill": {
                ...
                if (!meta)
                    throw new Error(`no verified supervisor recorded for ${target}`);
                const verified = verifiedSupervisorProcess({
                    busHome: BUS_HOME,
                    port: this.instancePort,
                    applicationRoot: APPLICATION_ROOT,
            ...
            case "/state/wait": {
                ...
                if (this.stateRevision > sinceRevision)
                    return { revision: this.stateRevision, changed: true };
...
export async function startBroker(options = {}) {
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            const status = error instanceof AuthError ? 401 : error instanceof PermissionError ? 403 : error instanceof ConflictError ? 409 : String(error.message).startsWith("no route") ? 404 : 400;
