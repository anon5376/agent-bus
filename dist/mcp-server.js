...
server.tool("bus_record_usage", "Report the tokens your last turn consumed. The broker prices them; you do not send a cost. Call it after finishing a turn so swarm spend is attributable per agent instead of vanishing into your own session.", {
...
}, async ({ input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, reasoning_tokens, task_id, model }) => guarded(async () => {
    const { recorded, event } = await authCall("/usage/record", {
    ...
    if (!recorded || !event)
        return "Nothing recorded — the report totalled zero tokens.";
...
server.tool("bus_usage", "Show what the swarm has spent over a trailing window, broken down by agent, model and provider.", {
    window_hours: z.number().positive().max(2160).default(5),
}, async ({ window_hours }) => guarded(async () => {
    const { summary } = await brokerCall("/usage/summary", {
        windowMs: Math.round(window_hours * 3_600_000),
    });
    if (!summary.totals.events)
        return `No usage recorded in the last ${window_hours}h.`;
    const line = (bucket) => `  ${bucket.label.padEnd(24)} ${bucket.totalTokens.toLocaleString().padStart(12)} tok  $${bucket.costUSD.toFixed(4)}` +
