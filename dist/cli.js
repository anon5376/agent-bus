...
async function renderState() { const state = await brokerCall("/state", {}); const out = []; out.push("\x1b[1mAGENTS\x1b[0m"); for (const agent of state.roster) {
    ...
    out.push(`  ${marker} ${String(agent.id).padEnd(12)} ${String(agent.role).padEnd(15)} ${String(agent.status).padEnd(9)} ${agent.family}/${agent.model} via ${agent.harness}${agent.currentTaskId ? ` · ${agent.currentTaskId}` : ""}`);
} out.push("\n\x1b[1mRUNS\x1b[0m"); if (!state.runs.length)
    out.push("  (none)"); for (const run of state.runs)
    out.push(`  ${run.id}  ${run.status.padEnd(9)} ${run.projectRoot} · ${run.goal.slice(0, 90)}`); out.push("\n\x1b[1mTASK GRAPH\x1b[0m"); const open = state.tasks.filter((t) => !["accepted", "failed", "cancelled"].includes(t.state)); if (!open.length)
    out.push("  (no open tasks)"); for (const task of open) {
...
async function main() {
    ...
    switch (command) {
        ...
        case "run": {
            ...
            const response = await brokerCall("/run/create", { token: operatorToken(), projectRoot: workdir, goal, role: flag("--role") ?? "manager", network: !hasFlag("--no-network") });
        ...
        case "usage": {
            ...
            for (const agent of state.roster.filter((x) => x.id !== "operator")) {
                const u = agent.usage;
                console.log(`${agent.id.padEnd(14)} ${agent.provider.padEnd(10)} ${agent.harness.padEnd(10)} ${u.turns} turns · ${u.totalTokens.toLocaleString()} tok · ${Math.round(u.latencyMs / 1000)}s${u.costUSD ? ` · $${u.costUSD.toFixed(4)}` : ""}`);
            }
            return;
        }
        case "send": {
