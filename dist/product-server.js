...
import { DEFAULT_CONFIG_PATH, loadConfig, resolveAgent, validateConfig, } from "./config.js";
...
class BrowserSessions {
    ...
    exchange(ticket) {
        ...
        if (!expiresAt || expiresAt <= Date.now())
            throw new BrowserAuthError("login ticket is invalid or expired");
...
function requireSameOrigin(req) {
    ...
    if (!origin || !host)
        throw new BrowserAuthError("same-origin request required");
...
function errorStatus(error) {
    ...
    if (message.startsWith("unauthorized"))
        return 401;
    if (message.includes("not authorized") || message.startsWith("only "))
        return 403;
    if (message.startsWith("no route") || message.startsWith("unknown run") || message.startsWith("unknown task"))
        return 404;
    if (message.includes("already") || message.includes("not submitted") || message.includes("not eligible") || message.includes("stop it before editing"))
        return 409;
    return 400;
}
function operatorToken(path) {
    const token = readTokenFile(path);
    if (!token)
        throw new Error(`operator token missing at ${path}`);
    return token;
}
function verifyOperatorToken(service, token) {
    const identity = token ? service.store.identityByTokenHash(hashToken(token)) : null;
    if (!identity || identity.authority !== "operator")
        throw new BrowserAuthError("invalid operator credential");
}
...
function saveAgent(configPath, body) {
...
function executionConfigSnapshot(config, id) {
...
function supervisedExecutionChanges(service, before, after) {
    const changed = [];
    for (const [id] of service.supervisorMeta) {
        try {
            if (!isDeepStrictEqual(executionConfigSnapshot(before, id), executionConfigSnapshot(after, id)))
                changed.push(id);
        }
        catch {
            changed.push(id);
...
function applyConfig(service, config) {
    ...
    for (const [id, definition] of Object.entries(config.agents)) {
        ...
        if (identity)
            service.store.upsertIdentity({ ...identity, authority: definition.authority, permissions: definition.permissions, updatedAt: Date.now() });
    }
...
function storedProjects(service) {
    try {
        return JSON.parse(service.store.getMeta(PROJECT_META_KEY) ?? "[]");
    }
    catch {
        return [];
    }
}
function projects(service) {
    const map = new Map();
    for (const project of storedProjects(service))
...
async function ensureAgentToken(service, operatorTokenPath, id) {
    const path = agentTokenPath(id);
    const existing = readTokenFile(path);
    if (existing)
        return existing;
    const token = operatorToken(operatorTokenPath);
    let result = await service.handle("/agent/provision", { token, id, rotate: false });
    if (!result.token)
        result = await service.handle("/agent/provision", { token, id, rotate: true });
    if (!result.token)
        throw new Error(`could not provision ${id}`);
    writePrivateToken(path, result.token);
    return result.token;
}
async function startAgent(service, operatorTokenPath, configPath, id, requested) {
    if (!configPath)
        throw new Error("agent supervision is unavailable with an in-memory config");
    ...
    if (live)
        return { id, started: false, pid: live.pid, message: "already supervised" };
...
async function stopRun(service, token, runId, reason) {
...
async function handleApi(req, res, pathname, params, service, sessions, operatorTokenPath, configPath, sessionTtlMs, eventStreams) {
    ...
    if (pathname === "/api/agents" && req.method === "POST") {
        ...
        const body = await readJson(req);
        const id = String(body.id ?? "").trim();
        if (service.supervisorMeta.has(id))
            throw new Error(`agent ${id} is supervised; stop it before editing its configuration`);
    ...
    if (pathname === "/api/integrations" && req.method === "POST") {
        if (!configPath)
            throw new Error("integration editing is unavailable with an in-memory config");
        ...
        if (affected.length)
            throw new Error(`integration update changes supervised agent execution (${affected.join(", ")}); stop it before editing configuration`);
    ...
    if (pathname === "/api/agents/stop-all" && req.method === "POST") {
        ...
        for (const [id] of [...service.supervisorMeta]) {
            try {
                results.push({ id, ...await service.handle("/kill", { token, agentId: id }) });
            }
            catch (error) {
...
export async function startProductServer(options = {}) {
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            if (req.method !== "POST")
                return sendJson(res, 405, { error: "legacy broker routes require POST" });
            return sendJson(res, 200, await service.handle(pathname, await readJson(req)));
        }
        catch (error) {
            return sendJson(res, errorStatus(error), { error: error.message });
        }
    });
...
//# sourceMappingURL=product-server.js.map