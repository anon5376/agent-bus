...
function unixBin(command, extras = []) {
    return [
        `~/.local/bin/${command}`,
...
export function catalogProviderDefinition(entry, enabled = false) {
    return {
        id: entry.id,
...
export function catalogHarnessDefinition(entry, command, enabled = false) {
...
export function catalogModelDefinition(entry, seed, enabled = false) {
    return {
        id: seed.id,
        provider: entry.id,
