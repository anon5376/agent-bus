...
import { PROVIDER_CATALOG, catalogEntry, catalogHarnessDefinition, catalogModelDefinition, catalogProviderDefinition, } from "./provider-catalog.js";
...
export function resolveBinary(command, knownPaths = [], explicit) {
    if (explicit?.trim()) {
    ...
    for (const dir of pathDirs()) {
        ...
        if (isExecutableFile(candidate) || existsSync(candidate))
            return candidate;
    }
...
async function probeEntry(entry, explicitCommand) {
...
export async function scanProviders(config, overrides = {}) {
    const rows = [];
    const seen = new Set();
...
export function mergeCatalogProvider(config, providerId, options = {}) {
    const entry = catalogEntry(providerId);
    if (!entry)
        throw new Error(`unknown catalog provider: ${providerId}`);
    const enabled = options.enabled !== false;
    const command = options.command?.trim() || config.harnesses[entry.harnessId]?.command || entry.binaries[0]?.command;
    if (!command)
        throw new Error(`no command for provider ${providerId}`);
    ...
    for (const seed of entry.models) {
        if (next.models[seed.id]) {
            if (enabled)
                next.models[seed.id].enabled = true;
            continue;
        }
...
export function applyFoundProviders(config, scans) {
    let next = structuredClone(config);
    const added = [];
    for (const scan of scans) {
        if (!scan.cliFound || scan.id === "ollama")
            continue;
