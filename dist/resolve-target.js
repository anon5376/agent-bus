...
export function resolveDelegateTarget(config, input) {
    ...
    if (exactAgent) {
        if (!config.agents[exactAgent])
            throw new Error(`unknown agent "${exactAgent}"`);
        return {
            exactAgent,
    ...
    const agents = enabledAgents(config).filter((agent) => {
        if (providers.length && !providers.includes(agent.modelDefinition.provider))
            return false;
        if (harness && agent.harnessDefinition.id !== harness && agent.harnessDefinition.adapter !== harness)
            return false;
        if (exactModel && exactModel !== agent.modelDefinition.id && exactModel !== agent.modelDefinition.exactModel)
            return false;
