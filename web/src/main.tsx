...
function App(){
  ...
  return <div className="app" data-agent-bus-mounted="true">
    ...
    <main className="main">
      <div className="run-head">{currentRun?<><div><span className="eyebrow">{currentRun.status} · {currentRun.id}</span><h1>{currentRun.goal}</h1><p>{currentRun.projectRoot}</p></div><div className="row"><button onClick={()=>setModal("message")}>Message team</button>{currentRun.status==="active"&&<button className="danger subtle" onClick={stopRun}>Stop run</button>}</div></>:<div><span className="eyebrow">CONTROL ROOM</span><h1>No run selected</h1><p>Create a run to start orchestrating agents.</p></div>}</div>
      ...
      <div className="workspace"><Panel title="Task graph" badge={tasks.length}>{tasks.map(t=><button className={`task depth-${Math.min(6,Math.max(0,t.depth||0))}`} key={t.id} onClick={()=>{setTaskId(t.id);setModal("task")}}><span className={`state ${t.state}`}/><div><b>{t.title}</b><small>{t.state} · {t.assignee||"unassigned"} · {t.role}/c{t.complexity} · r{t.round}{t.attempts?` · ${t.attempts} retry`:""}{t.dependencyIds?.length?` · deps ${t.dependencyIds.length}`:""}</small>{t.routing?.reason&&<em>{t.routing.reason}</em>}</div></button>)}</Panel>
        <Panel title="Live events & messages" badge={messages.length}>{messages.slice(0,250).map(m=><div className="message" key={m.id}><div><b>{m.from}</b><span>→ {m.to} · {m.type} · {new Date(m.ts).toLocaleTimeString()}</span></div><strong>{m.subject}</strong><p>{m.body}</p></div>)}</Panel></div>
    </main>
    <aside className="right"><div className="pane-head"><span>Agents</span><button onClick={()=>setModal("agent")}>Add</button></div>{agents.map(a=><AgentCard key={a.id} agent={a} catalog={catalog} project={currentRun?.projectRoot||projects[0]?.path||""} onToast={note} onEdit={()=>{setTaskId(a.id);setModal("agent")}}/> )}</aside>
...
function AgentCard({agent,catalog,project,onToast,onEdit}:{agent:Agent;catalog:Catalog|null;project:string;onToast:(s:string)=>void;onEdit:()=>void}){const active=Boolean(agent.supervisorPid);const modelDef=catalog?.models?.[agent.model];const act=async()=>{try{if(active)await post(`/api/agents/${agent.id}/stop`);else await post(`/api/agents/${agent.id}/start`,{projectRoot:project});onToast(active?`${agent.id} stopped`:`${agent.id} starting`)}catch(e:any){onToast(e.message)}};return <div className="agent-card"><div className="agent-top"><div><span className={`dot ${agent.status==="working"?"warn":agent.status==="offline"?"":"ok"}`}/><b>{agent.id}</b></div><span>{agent.stalled?"stalled":agent.status}</span></div><small>{agent.provider} · {agent.family}/{agent.model}<br/>{modelDef?.exactModel?`exact: ${modelDef.exactModel} · `:""}{agent.harness} · {agent.role}</small><div className="agent-usage"><span>{fmtTokens(agent.usage?.totalTokens||0)} tok</span><span>{agent.usage?.turns||0} turns</span><span>{Math.round((agent.usage?.latencyMs||0)/1000)}s</span></div><div className="row"><button onClick={act}>{active?"Stop":"Start"}</button><button onClick={onEdit}>Edit</button></div></div>}
