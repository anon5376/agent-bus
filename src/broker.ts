...
const tokensToId = new Map<string, string>(); // token -> agent id
const idToToken = new Map<string, string>(); // agent id -> its stable token
...
const supervisorMeta = new Map<string, { workdir: string; cli: string }>();
...
function caller(b: any): string {
  const tok = typeof b?.token === "string" ? b.token : "";
  const id = tokensToId.get(tok);
  if (!id) throw new AuthError("unauthorized: missing or invalid token");
  return id;
}
...
function audit(kind: string, data: unknown): void {
  try {
    appendFileSync(
      LOG_PATH,
...
function mailbox(agentId: string): Message[] {
...
function touch(agentId: string, status?: AgentStatus): Agent | undefined {
  ...
  return [...agents.values()]
    .map((a) => {
      ...
      return {
         ...
         */
        stalled:
          pending > 0 && !blocked && effectiveStatus(a) !== "working",
...
function resolveRecipients(from: string, to: string): string[] {
...
type Handler = (body: any) => Promise<unknown> | unknown;
...
const routes: Record<string, Handler> = {
  "/register": (b) => {
    ...
    if (b.description) agent.description = String(b.description);
    if (b.harness) agent.harness = String(b.harness);
    if (b.auth) agent.auth = String(b.auth);
    agent.lastSeen = Date.now();
    if (effectiveStatus(agent) === "offline") agent.status = "idle";
    agents.set(id, agent);
    mailbox(id);
  ...
  "/status": (b) => {
    const id = caller(b);
    const agent = touch(id, b.status as AgentStatus | undefined);
  ...
  "/presence": (b) => {
    ...
    if (Number.isFinite(pid) && pid > 0) supervisorPids.set(id, pid);
    if (b.workdir || b.cli) {
  ...
  "/send": (b) => {
    ...
    if (recipients.length === 0) throw new Error("no recipients resolved");
    ...
    for (const to of recipients) {
      ...
      const msg = makeMessage({
        from,
        to,
        type: (b.type ?? "info") as Message["type"],
  ...
  "/peek": (b) => {
    ...
    touch(agentId);
    const box = mailbox(agentId);
    if (b.drain === false) return { messages: box.slice(), timedOut: false };
  ...
  "/task/create": (b) => {
    ...
    if (!agents.has(assignee)) {
      throw new Error(
        `unknown assignee "${assignee}" — registered agents: ${[...agents.keys()].join(", ") || "(none)"}`,
      );
    }
    const now = Date.now();
    const task: Task = {
  ...
  "/task/submit": (b) => {
    ...
    if (actor !== task.assignee && actor !== OPERATOR_ID) {
      throw new AuthError(
        `only ${task.assignee} may submit ${task.id} (you are ${actor})`,
      );
    }
    touch(actor, "idle");
  ...
  "/task/review": (b) => {
    ...
    if (actor !== task.assigner && actor !== OPERATOR_ID) {
      throw new AuthError(
        `only ${task.assigner} or the operator may review ${task.id} (you are ${actor})`,
      );
    }
    touch(actor);
    const accepted = Boolean(b.accepted);
    const state: TaskState = accepted ? "accepted" : "changes_requested";
    task.state = state;
    task.updatedAt = Date.now();
    if (!accepted) task.round += 1;
    task.history.push({
      ts: Date.now(),
      actor,
      kind: "reviewed",
    ...
    if (worker) worker.currentTaskId = accepted ? null : task.id;
  ...
  "/task/list": (b) => {
    ...
    const list = [...tasks.values()]
      ...
      .filter((t) =>
        open ? !["accepted", "cancelled"].includes(t.state) : true,
      )
  ...
  "/task/cancel": (b) => {
    ...
    if (actor !== task.assigner && actor !== OPERATOR_ID) {
      throw new AuthError(
        `only ${task.assigner} or the operator may cancel ${task.id} (you are ${actor})`,
      );
    }
    ...
    const worker = agents.get(task.assignee);
    if (worker && worker.currentTaskId === task.id) worker.currentTaskId = null;
    deliver(
      makeMessage({
        from: actor,
        to: task.assignee,
  ...
  "/kill": (b) => {
    const id = caller(b);
    if (id !== OPERATOR_ID) throw new AuthError("only the operator may kill agents");
    const target = String(b.agentId);
    const pid = supervisorPids.get(target);
    if (!pid) throw new Error(`no supervisor pid recorded for "${target}"`);
    let killed = false;
    try {
...
function initOperator(): void {
  ...
  try {
    ...
    process.stderr.write(
      `could not write operator token to ${OPERATOR_TOKEN_PATH}: ${(err as Error).message}\n`,
    );
  }
}

export function startBroker(): Promise<void> {
  initOperator();
  const server = createServer(async (req, res) => {
    ...
    if (!handler) return send(res, 404, { error: `no route ${path}` });
    try {
      const body = await readBody(req);
      send(res, 200, await handler(body));
    } catch (err) {
      const code = err instanceof AuthError ? 401 : 400;
      send(res, code, { error: (err as Error).message });
    }
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(BUS_PORT, BUS_HOST, () => {
      process.stderr.write(
        `agent-bus broker listening on http://${BUS_HOST}:${BUS_PORT} (log: ${LOG_PATH})\n`,
      );
      resolve();
    });
  });
