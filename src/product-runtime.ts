...
export interface ProductArtifactManifest {
  ...
  moduleRoot: string;
  staticRoot: string;
  index: ArtifactFile | null;
  scripts: ArtifactFile[];
  styles: ArtifactFile[];
  staticFiles: ArtifactFile[];
  uiBuilt: boolean;
}
...
function artifactFile(path: string, root: string, browserVisible: boolean): ArtifactFile {
...
function referencedUrls(indexHtml: string, extension: ".js" | ".css"): string[] {
...
function byUrl(files: ArtifactFile[], url: string): ArtifactFile | null {
  const pathname = url.split(/[?#]/, 1)[0];
  return files.find((file) => file.url === pathname) ?? null;
}

export function runtimeApplicationRoot(): string {
...
export function productArtifactManifest(staticRoot = defaultStaticRoot()): ProductArtifactManifest {
  ...
  if (!hashInputs.length) hash.update(`${PRODUCT_NAME}:${PRODUCT_PROTOCOL_VERSION}`);
...
export function productBuildId(staticRoot = defaultStaticRoot()): string {
