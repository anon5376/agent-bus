...
function isCurrentHealth(health: ProductHealth | null): boolean {
...
function brokerLogTail(maxBytes = 16_000): string {
  try { return readFileSync(BROKER_LOG, "utf8").slice(-maxBytes).trim(); }
  catch { return ""; }
}
...
export async function runtimeDiagnostic(): Promise<Record<string, unknown> | null> {
  try {
...
export async function verifyServedDashboard(): Promise<void> {
  ...
  for (const asset of assets) {
    ...
    if (sha256(body) !== asset.sha256) throw new Error(`served ${pathname} does not match the installed artifact`);
  }
}
