import type { BusState, Task } from "./protocol.js";

const CLOSED: Task["state"][] = ["accepted", "failed", "cancelled"];
...
export function renderBusState(state: BusState): string {
  const out: string[] = [];
...
export function renderUsage(state: BusState): string {
  return state.roster
