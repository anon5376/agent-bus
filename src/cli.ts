...
case "run":{const workdir=resolve(process.argv[3]??"");const goal=flag("--goal");if(!process.argv[3]||!goal)throw new Error("usage: agent-bus run <project-dir> --goal \"Implement X\" [--role manager] [--no-autostart]");const configPath=process.env.AGENT_BUS_CONFIG??DEFAULT_CONFIG_PATH;await ensureBrokerStarted();const config=loadConfig(configPath);const started:{id:string;pid:number}[]=[];if(!hasFlag("--no-autostart")){for(const agent of enabledAgents(config).filter(a=>a.autoStart)){await provisionAgent(agent.id);started.push({id:agent.id,pid:startSupervisor(agent.id,workdir,configPath)})}if(started.length)await new Promise(r=>setTimeout(r,900))}const response=await brokerCall<{run:Run;rootTask:Task}>("/run/create",{token:operatorToken(),projectRoot:workdir,goal,role:flag("--role")??"manager",network:!hasFlag("--no-network")});console.log(`run: ${response.run.id}`);console.log(`project: ${response.run.projectRoot}`);console.log(`root task: ${response.rootTask.id} → ${response.rootTask.assignee}`);console.log(`routing: ${response.rootTask.routing?.reason??"unavailable"}`);if(started.length)console.log(`supervisors: ${started.map(x=>`${x.id}:${x.pid}`).join(", ")}`);console.log(`dashboard: ${DASHBOARD_URL}`);return}
