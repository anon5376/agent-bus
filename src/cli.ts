...
async function renderState(): Promise<string> {
  ...
  if (state.roster.length === 0) out.push("  (none registered yet)");
  for (const a of state.roster) {
    out.push(
      `  ${a.stalled ? "\x1b[31m▲\x1b[0m" : (DOT[a.status] ?? "?")} ${a.id.padEnd(12)} ${a.role.padEnd(8)} ` +
  ...
  const open = state.tasks.filter(
    (t: any) => !["accepted", "cancelled"].includes(t.state),
  );
  if (open.length === 0) out.push("  (no open tasks)");
  for (const t of open) {
    out.push(
      `  ${t.id}  \x1b[1m${t.state}\x1b[0m  ${t.assigner} → ${t.assignee}  r${t.round}  ${t.title}  \x1b[90m${fmtAgo(t.updatedAt)}\x1b[0m`,
    );
  }
  const closed = state.tasks.length - open.length;
  if (closed) out.push(`  \x1b[90m(+${closed} closed)\x1b[0m`);
  return out.join("\n");
}
...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "usage": {
      ...
      const state = await brokerCall<any>("/state", {});
      const bySub: Record<string, { tokens: number; cost: number; turns: number; agents: string[] }> = {};
      console.log("\x1b[1mUSAGE BY AGENT\x1b[0m");
      for (const a of state.roster) {
        if (a.id === "operator") continue;
        const u = a.usage ?? { turns: 0, tokens: 0, costUSD: 0 };
        const sub = a.auth || "unknown";
        const g = (bySub[sub] ??= { tokens: 0, cost: 0, turns: 0, agents: [] });
        g.tokens += u.tokens; g.cost += u.costUSD; g.turns += u.turns; g.agents.push(a.id);
        const cost = u.costUSD ? ` · $${u.costUSD.toFixed(4)}` : "";
        console.log(
          `  ${a.id.padEnd(10)} ${String(a.harness ?? "?").padEnd(9)} ${String(a.model).padEnd(12)}` +
    ...
    case "watch": {
      ...
      const tick = async () => {
        const body = await renderState().catch(
          (e) => `broker unreachable: ${e.message}`,
        );
        process.stdout.write(
          `\x1b[2J\x1b[H\x1b[1magent-bus\x1b[0m  ${new Date().toLocaleTimeString()}\n\n${body}\n`,
        );
      };
      await tick();
        ...
        createReadStream(path, { start: pos, end: size })
          .on("data", (chunk) => {
            for (const line of chunk.toString().trim().split("\n")) {
              ...
              const m = e.data;
              console.log(
                `\x1b[90m${new Date(m.ts).toLocaleTimeString()}\x1b[0m \x1b[1m${m.from}\x1b[0m → ${m.to} \x1b[36m[${m.type}]\x1b[0m ${m.subject}`,
              );
            }
          })
          .on("end", () => (pos = size));
