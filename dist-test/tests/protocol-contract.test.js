...
import { ProtocolError, emptyUsage, parseBusState, } from "../src/protocol.js";
...
async function brokerFixture(config = testConfig()) {
...
test("today's deployed /state body is a named contract error, not a TypeError on .length", () => {
    assert.throws(() => { void TODAY_DEPLOYED_STATE.runs.length; }, { name: "TypeError" });
    assert.throws(() => parseBusState(TODAY_DEPLOYED_STATE), (error) => {
    ...
    assert.throws(() => parseBusState({ ...TODAY_DEPLOYED_STATE, runs: [], telemetry: [], pathLeases: [], revision: 1, configIdentity: { path: null, digest: "x" } }), /missing field roster\[0\]\.family/);
});
...
test("broker /state satisfies BusState, including empty and stale live roster", async (t) => {
    ...
    for (const agent of Object.values(emptyConfig.agents))
        agent.enabled = false;
    ...
    assert.ok(Array.isArray(agentsAlias.body.roster));
    ...
    const hoursAgo = (id) => live.find((row) => row.id === id).lastSeenSecondsAgo;
