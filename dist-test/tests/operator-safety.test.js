...
async function connectOperator(env) {
...
async function call(client, name, args = {}) {
    ...
    if (result.isError)
        throw new Error(result.content?.[0]?.text ?? `MCP ${name} failed`);
...
async function dashboardSession(url, operatorToken) {
...
async function waitForRosterPid(client, agentId, pid, timeoutMs = 5_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const status = await call(client, "agent_bus_status");
        const row = (status.roster ?? []).find((entry) => entry.id === agentId);
        if (Number(row?.supervisorPid) === pid)
            return;
        await new Promise((resolveDelay) => setTimeout(resolveDelay, 80));
    }
...
test("stale supervisors self-heal and agent_bus_wait wakes for operator questions", {
    ...
    try {
        ...
        const staleRow = (staleStatus.roster ?? []).find((entry) => entry.id === "fake-small");
        ...
        assert.equal(attention.messages?.some((message) => message.type === "question" && message.subject === "Need operator input"), true);
