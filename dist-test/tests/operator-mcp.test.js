...
async function dashboardState(url, token) {
...
test("operator MCP drives the same broker state as the dashboard and remains separate from worker MCP", { timeout: 90_000 }, async () => {
    ...
    try {
        ...
        assert.equal(state.runs.some((run) => run.id === execution.runId), true, "dashboard must show MCP-created run id");
        assert.equal(state.tasks.some((task) => task.id === execution.rootTaskId), true, "dashboard must show MCP-created task id");
