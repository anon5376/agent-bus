...
test("single localhost server serves SPA but does not authenticate direct visitors", async (t) => {
    ...
    const page = await fetch(`${f.handle.url}/`);
    assert.equal(page.status, 200);
    assert.match(await page.text(), /Agent Bus React/);
    assert.equal(page.headers.get("set-cookie"), null);
    assert.equal((await fetch(`${f.handle.url}/api/state`)).status, 401);
    const bad = await fetch(`${f.handle.url}/dashboard/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: "wrong" }) });
...
test("one-time CLI ticket exchanges for HttpOnly session and cannot be replayed", async (t) => {
    ...
    const replay = await fetch(`${f.handle.url}/api/session`, { method: "POST", headers: { origin: f.handle.url, "content-type": "application/json" }, body: JSON.stringify({ ticket }) });
...
test("browser API persists projects/runs, requires same-origin mutations, and can stop a run", async (t) => {
    ...
    const noOrigin = await fetch(`${f.handle.url}/api/projects`, { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ path: f.root }) });
    assert.equal(noOrigin.status, 401);
    const project = await fetch(`${f.handle.url}/api/projects`, { method: "POST", headers: { cookie, origin: f.handle.url, "content-type": "application/json" }, body: JSON.stringify({ path: f.root }) });
    ...
    assert.equal(runRes.status, 200, runText);
    ...
    const stopped = JSON.parse(stopText);
    assert.equal(stopped.run.status, "cancelled");
    assert.ok(stopped.tasks.every((task) => ["accepted", "failed", "cancelled"].includes(task.state)));
    const projectList = await fetch(`${f.handle.url}/api/projects`, { headers: { cookie } });
    assert.ok((await projectList.json()).projects.some((p) => p.path === f.root));
});
test("SSE is session protected and emits real broker snapshots", async (t) => {
    const f = await fixture();
    t.after(() => f.handle.close());
    assert.equal((await fetch(`${f.handle.url}/api/events`)).status, 401);
    const cookie = await login(f);
    const controller = new AbortController();
    const response = await fetch(`${f.handle.url}/api/events`, { headers: { cookie }, signal: controller.signal });
    assert.equal(response.status, 200);
    assert.match(response.headers.get("content-type") ?? "", /text\/event-stream/);
    const reader = response.body.getReader();
    const first = await reader.read();
    assert.match(new TextDecoder().decode(first.value), /connected|snapshot/);
    controller.abort();
});
test("agent editor updates config and live catalog without broker restart", async (t) => {
    ...
    assert.equal(body.agents["fake-small"].description, "edited live");
    assert.equal(body.agents["fake-small"].autoStart, true);
    const disk = JSON.parse(readFileSync(f.configPath, "utf8"));
    assert.equal(disk.agents["fake-small"].description, "edited live");
});
...
test("agent edits cannot mutate a shared model definition", async (t) => {
    ...
    const createPeerText = await createPeer.text();
    assert.equal(createPeer.status, 200, createPeerText);
    ...
    assert.match(responseText, /model definitions are shared/i);
...
test("running supervisors reject agent configuration edits instead of creating split-brain state", async (t) => {
    ...
    assert.match(responseText, /stop it before editing/i);
    const after = JSON.parse(readFileSync(f.configPath, "utf8"));
...
test("malformed JSON is rejected without crashing the product server", async (t) => {
    ...
    t.after(() => f.handle.close());
    const cookie = await login(f);
    const response = await fetch(`${f.handle.url}/api/messages`, { method: "POST", headers: { cookie, origin: f.handle.url, "content-type": "application/json" }, body: "{" });
    assert.equal(response.status, 400);
    assert.equal((await fetch(`${f.handle.url}/health`)).status, 200);
});
