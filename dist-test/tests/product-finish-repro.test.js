...
async function waitForTask(url, taskId, states, timeoutMs = 10_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const response = await post(url, "/task/get", { taskId });
        if (states.includes(response.body.task?.state))
...
test("direct public supervise resolves the running broker config instead of conflicting project-local config", { timeout: 25_000 }, async () => {
    ...
    for (const [id, agent] of Object.entries(configA.agents))
        agent.enabled = id === "fake-small";
    ...
    try {
        ...
        const provision = await post(url, "/agent/provision", { token: operator, id: "fake-small" });
        ...
        while (Date.now() < deadline) {
            const roster = await post(url, "/roster", {});
            if (roster.body.roster?.some((agent) => agent.id === "fake-small" && agent.status !== "offline"))
                break;
            await new Promise((resolve) => setTimeout(resolve, 80));
        }
        const created = await post(url, "/task/create", {
            token: operator,
            assignee: "fake-small",
        ...
        if (supervisor?.pid && alive(supervisor.pid)) {
            try {
...
test("upgrading the merged PR5 target listener is positively identified", () => {
    const owner = classifyPortOwner(4242, "/opt/homebrew/bin/node /Users/me/code/agent-bus/dist/cli.js broker", {
        ok: true,
        pid: 4242,
        product: "agent-bus",
        productProtocol: 2,
        ...
        tasks: 1,
        runs: 1,
    }, "pr6-build", false, { busHome: "/Users/me/.agent-bus", applicationRoot: "/Users/me/.agent-bus/app/current" });
...
test("upgrading the merged PR5 checkout broker replaces only the requested target listener", { timeout: 30_000 }, async () => {
  ...
  `);
    ...
    try {
        ...
        while (pr5.pid && alive(pr5.pid) && Date.now() < deadline)
            await new Promise((resolve) => setTimeout(resolve, 80));
        ...
        runCli(envA, "stop");
        runCli(envB, "stop");
        if (pr5.pid && alive(pr5.pid)) {
            try {
                process.kill(-pr5.pid, "SIGKILL");
            }
            catch { }
        }
        if (peer.pid && alive(peer.pid)) {
            try {
...
test("integration guard compares candidate config against live broker state, not drifted disk", async (t) => {
    ...
    handle.service.supervisorMeta.set("fake-small", { pid: 424242, childPid: null, workdir: root, cli: "fake", startedAt: Date.now() });
...
test("agent-supplied supervisor PID cannot make operator kill an unrelated process", { timeout: 10_000 }, async (t) => {
    ...
    const provision = await post(handle.url, "/agent/provision", { token: operator, id: "fake-small" });
    const victim = spawn(process.execPath, ["-e", "setInterval(()=>{},1000)"], { stdio: "ignore", detached: true });
    try {
        assert.ok(victim.pid);
        const registered = await post(handle.url, "/register", { token: provision.body.token, id: "fake-small", pid: victim.pid, workdir: root, cli: "fake" });
        ...
        if (victim.pid && alive(victim.pid)) {
            try {
...
test("PID start-fingerprint mismatch prevents operator kill after ownership record becomes stale", { timeout: 20_000 }, async () => {
    ...
    try {
        ...
        while (Date.now() < deadline) {
            const roster = await post(url, "/roster", {});
            verified = roster.body.roster?.some((agent) => agent.id === "fake-small" && agent.supervisorPid === supervisor.pid);
            if (verified)
                break;
            await new Promise((resolve) => setTimeout(resolve, 80));
        }
        ...
        const killed = await post(url, "/kill", { token: operator, agentId: "fake-small" });
...
test("genuine fingerprint-verified supervisor remains stoppable by operator", { timeout: 20_000 }, async () => {
    ...
    try {
        ...
        while (Date.now() < deadline) {
            const roster = await post(url, "/roster", {});
            if (roster.body.roster?.some((agent) => agent.id === "fake-small" && agent.supervisorPid === supervisor.pid))
                break;
            await new Promise((resolve) => setTimeout(resolve, 80));
        }
        const killed = await post(url, "/kill", { token: operator, agentId: "fake-small" });
        ...
        while (alive(supervisor.pid) && Date.now() < downDeadline)
            await new Promise((resolve) => setTimeout(resolve, 80));
