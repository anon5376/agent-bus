...
| xAI / Grok | Disabled `grok` harness entry | Provider-specific account | Unsupported by default | The interface is present for mechanical future work, but the default is disabled because no official subscription CLI path was validated here. |
...
`agent-bus doctor` checks executable availability and version output. It deliberately does not infer login state, subscription entitlement, remaining quota or model access from the presence of a binary.
